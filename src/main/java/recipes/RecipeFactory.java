...
public class RecipeFactory {
    ...
    private static Recipe createRecipeFromJamila(URL recipeURL) {
        ...
        try {
            ...
            // add the duration
            newRecipe.setDuration(getDuration(doc), "hours");
    ...
    private static URL getVideoURL(Document document) {
        Element iframe = document.select("div.wprm-recipe-video").select("iframe").first();
        if (iframe != null) {
            String src = iframe.attr("data-litespeed-src");
    ...
    private static double getDuration(Document doc) {
        ...
        double totalTime = 0;
        try  {
            totalTime = Double.parseDouble(hourString);
        } catch (NumberFormatException e) {
            totalTime = 0;
        }
        try {
