...
import com.bitbites.bitbites2.backend.exceptions.GroceryItemException;
import com.bitbites.bitbites2.backend.exceptions.GroceryListException;
...
public class GroceryListsRepository implements BitBitesRepository<GroceryListModel>{
    ...
    public int add(Connection connection, GroceryListModel item) {
        ...
        try (PreparedStatement ps = connection.prepareStatement(sql, PreparedStatement.RETURN_GENERATED_KEYS)) {
            ps.setLong(1, item.recipeId() == 0 ? item.mealPlanId() : item.recipeId());
            ...
            AuditService.getInstance().logException(new GroceryListException("Cannot add grocery list to database." + e.getMessage()));
        }
        return 0;
    }
